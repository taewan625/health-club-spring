...
<html>
	<head>
		...
		<title>선택 가능 사물함 목록</title>
		<link rel="stylesheet" href="/css/styles.css">
	</head>
	
	<body>
		<h1 class="main-title"> 선택 가능 사물함 목록</h1>
		 ...
		 <div class="search-box search-box--small">
		   	...
		   	<div class="search-box__menus">
			 	<div id="searchBoxMenusConditions" name="searchBoxMenusConditions" class="search-box__menu--conditions">
					...
					<input type="hidden" id="searchText" name="searchText" class="osl-evt__search-condition search-condition" maxlength="2"/>
			 	</div> 	
		 	</div>
		...
		<div class="popup-btns">
			<button type="submit" id="save"   name="save"   class="popup-btn--save"> 선택 </button>
	...
	<script type="text/JavaScript">
		$(document).ready(function() {
			...
			$(document).on('click', '.osl-evt__table__row', function() {
			    ...
			    if (isTableCell) {
			        var radiobox = tableCell.parent().find('input[type="radio"][name="lockerId"]');
			        radiobox.prop('checked', true);
			    }
			});
			
			// 사물함 정보 선택 이벤트
			$('#save').on('click', fnSelectLocker);
		...
		*/
		...
		/**
		 * function : fnListAjax 
		 * function 설명 : 목록에서 수행하는 공통 Ajax 함수
		*/
		function fnListAjax(requestData) {
			 // userList와 페이징정보를 가지고 와서 화면을 동적으로 그리는 ajax
			 var returnValue = gfnAjaxPost('/cmm/cmm2000/cmm2000/getCmm2000LckList.do', requestData);
		...
		/**
		 * @function fnReRender : 실제로 html 화면을 새로 그려주는 함수
		 * @param  {Ojbect} : responseDatas, page
		 * @return void : html dom 형식만 변경
		*/
		function fnReRender(responseData, page) {
		    ...
		    lockersList.forEach(function(locker, index) {
		    	...
		    	// jQuery inner Html
		    	tableRow.html(
			    	'<div class="table-cell"><input type="radio" name="lockerId" value="' + locker.lockerId +',' + locker.lockerEndDate + '"/></div>'
			    	+ '<div class="table-cell">' + gfnDefenceXSS(locker.lockerId) + '</div>'
			    	+ '<div class="table-cell">' + gfnDefenceXSS(locker.userId) + '</div>'
			    	+ '<div class="table-cell">' + gfnDefenceXSS(locker.userName) + '</div>'
			    	+ '<div class="table-cell">' + gfnDefenceXSS(locker.lockerEndDate) + '</div>'
		    	);
		...
		/**
		 * @function fnSelectLocker 
		 * @description locker를 선택하는 함수
		 * @param 
		 * @return 
		*/
		function fnSelectLocker () {
		    ...
		    $('input[type="radio"][name="lockerId"]').each(function() {
		        // 현재 요소가 체크되어 있는지 확인
		        if (this.checked) {
		            // 체크된 경우
		
		            // 1. 사용중인 내용 알림
		            [lockerId, used] = $(this).val().split(',');
		
		            if (used !== '-') {
		                var answer = confirm('사용중인 회원이 존재합니다. 그래도 사용하실거면 "확인", 아니면 "취소" 버튼을 눌러주세요.');
		            ...
		            // 2. 사물함 선택
		            window.opener.document.querySelector('input[type="text"][id="lockerId"]').value = lockerId;
		            checked = true;
		            // each() 함수 종료
		            return false;
		        }
		    ...
		    // 사물함이 선택되지 않은 경우 알림 표시
		    checked ? window.close() : alert('사물함을 선택해주세요');
		};
	</script>
</html>